...
while read ARG1 ARG2 ARG3 ARG4 ARG5 ARG6 ARG7; do
	if [ "${ARG4%:*}" = "ESSID" ]; then
		...
		LXNM_ESSID=`asc2hex "$LXNM_ESSID"`
	elif [ "${ARG1}" = "Link" ]; then
		if [ A"${ARG2%:*}" = A"Quality" ]; then
			QUAL=${ARG2#*:}
		else
			QUAL=${ARG2#*=}
		fi
...
echo -e "+$LXNM_CMDID $LXNM_ENABLE\t$LXNM_PLUGED\t$LXNM_CONNECTED\t$LXNM_MAC\t$LXNM_IPADDR\t$LXNM_DEST\t$LXNM_BROADCAST\t$LXNM_NETMASK\t$LXNM_RX_BYTES\t$LXNM_RX_PACKETS\t$LXNM_TX_BYTES\t$LXNM_TX_PACKETS\t$LXNM_ESSID\t$LXNM_BSSID\t$LXNM_QUALITY"
